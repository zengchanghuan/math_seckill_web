          ...
          <div className="max-w-3xl mx-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <div className="space-y-1.5">
                  {['自考高等数学', '高中数学', '考研数学', '其它'].map((option) => (
                    <label key={option} className="flex items-center">
                      <input
                        type="radio"
                        name="preferredSubject"
                        value={option}
                        checked={formData.preferredSubject === option}
                        onChange={(e) => {
                          const value = e.target.value;
                          setFormData({
                            ...formData,
                            preferredSubject: value,
                            otherSubject: value !== '其它' ? '' : formData.otherSubject
                          });
                        }}
                        className="mr-2"
                      />
                ...
                {formData.preferredSubject === '其它' && (
                  <input
                    type="text"
                    placeholder="请输入你希望增加的科目"
                    value={formData.otherSubject}
                    onChange={(e) => setFormData({ ...formData, otherSubject: e.target.value })}
                    className="w-full mt-3 px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm"
                  />
                )}
              </div>
